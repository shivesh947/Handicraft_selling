       <!DOCTYPE html>
       <html>
<head>  
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Categories</title>
    <link rel="stylesheet" href="assets/bootstrap/css/bootstrap.min.css">
    <link rel="stylesheet" href="assets/fonts/font-awesome.min.css">
    <link rel="stylesheet" href="assets/fonts/ionicons.min.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Caveat+Brush">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Roboto+Slab:300,400|Roboto:300,400,700">
    <link rel="stylesheet" href="assets/css/-Fixed-Navbar-start-with-transparency-background-BS4-.css">
    <link rel="stylesheet" href="assets/css/Bootstrap-4-Navbar-with-Logo-Image-Brand-Name-and-Responsive-Menu.css">
    <link rel="stylesheet" href="assets/css/Footer-Dark.css">
    <link rel="stylesheet" href="assets/css/MUSA_navbar-metro.css">
    <link rel="stylesheet" href="assets/css/SO-Floating-Nav_v10.css">
    <link rel="stylesheet" href="assets/css/sticky-dark-top-nav-with-dropdown.css">
    <link rel="stylesheet" href="assets/css/styles.css">

    <link rel="stylesheet" href="assets/css/animatecss.css">
    <link rel="stylesheet" href="assets/css/hovercss.css">
    <link rel="stylesheet" href="assets/css/shakecss.css">
    <link rel="stylesheet" href="assets/css/mynew.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
</head>
<style>
* {
  box-sizing: border-box;
  }


body {
  font-family: Arial, Helvetica, sans-serif;
}

/* Float four columns side by side */
.column {
  float: left;
  width: 25%;
  padding: 0 10px;
  margin-top: 30px;
}
.column:hover{
box-shadow: 3px 3px 2px 2px grey;
transform: scale(1.1);
}

/* Remove extra left and right margins, due to padding */
.row {margin: 0 -5px;}

/* Clear floats after the columns */
.row:after {
  content: "";
  display: table;
  clear: both;
}

/* Responsive columns */
@media screen and (max-width: 600px) {
  .column {
    width: 100%;
    display: block;
    margin-bottom: 20px;
  }
}

/* Style the counter cards */
.card {
  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2);
  padding: 16px;
  text-align: center;
  background-color: #f1f1f1;
}
#data
{
    padding:30px;
    
}
</style>

 <script type="text/javascript" src="http://code.jquery.com/jquery-1.10.1.min.js"></script>
  
   <body background="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTpmA1z3W99mjNuUui3WzimnXK53OPoD07IWRWIxLV2tSHjfUpY" style="background-size:cover;">
    
    <div>
        <header style="background-image:url(data:image/jpeg;base64,);">
            <nav class="navbar navbar-dark navbar-expand-lg sticky-top border-bottom border-light" style="margin-top: -40px;">
                <div class="container-fluid"><a class="navbar-brand text-success" href="#header"><i class="fa fa-ticket fa-2x"></i>&nbsp;VIREKHA</a><button class="navbar-toggler collapsed" style="height: 25px; background-color: #f5c527;
                border: 2px solid black;" data-toggle="collapse" data-target="#navcol-1"><span></span><span></span><span></span></button>
                    <div class="collapse navbar-collapse p-4" id="navcol-1">
                        <ul class="nav navbar-nav ml-auto">
                            <li class="nav-item , hvr-float-shadow" role="presentation"><a class="nav-link" href="../home.php">Login & Signup</a></li>
                            <li class="nav-item , hvr-float-shadow" role="presentation"><a class="nav-link" href="#">Categories</a></li>
                            <li class="nav-item , hvr-float-shadow" role="presentation"><a class="nav-link" href="../About/About.php">About us</a></li>
                            <li class="nav-item , hvr-float-shadow" role="presentation"><a class="nav-link" href="../Portfolio/index.html">Portfolio</a></li>
                            <li class="nav-item , hvr-float-shadow" role="presentation"><a class="nav-link" href="#foot">Contact</a></li>
                            <li class="nav-item , hvr-float-shadow" role="presentation"><a class="nav-link" href="userfeedback.php">Feedback</a></li>
                      </ul>
                </div>
           </div>
           </nav>
        </header>
        <marquee><img src="assets/img/bike.png" height="100px"><img src="assets/img/statue.png" height="100px"><img src="assets/img/statue.png" height="100px"><img src="assets/img/statue.png" height="100px"></marquee>
        <style type="text/css">
         .navbar-dark .navbar-nav .nav-link
        	{
        		color: black;
        		font-size: 20px;
        	} 
        </style>
    </div>

<div id="slides">
	 <a href="Earthenitems.php"><img src="assets/img/download (1).jpg" id="oo" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (2).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (3).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (4).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (5).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (6).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (7).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (8).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (9).jpg" class="slidee"></a>
	 <a href="Earthenitems.php"><img src="assets/img/download (10).jpg" class="slidee"></a>

</div>

<style type="text/css">
	.slidee
	{
		display: none;
		width: 100%;
		height: 400px;
	}

	#oo
	{
		display: block;
   
	}

</style>
<script type="text/javascript">
	var i=0;
	function fun()
	{
		var a=document.getElementsByClassName('slidee');
		for(var j=0;j<a.length;j++)
		{
			a[j].style.display="none";
		}
		a[i].style.display="block";
		if(i==9)
		{
			i=0;
			fun();
		}
		i++;
		
	}
	setInterval(fun,1500);
</script>
<script type="text/javascript">
  function bigme(a)
  {
    var val=["no","Earthenitems.php","gifteditems.php","juteitems.php","lightitems.php"]
    document.body.innerHTML=" ";
    document.body.style.backgroundImage="url(https://cdn.dribbble.com/users/131684/screenshots/1368380/beingdad_v02.gif)";
    document.body.style.backgroundRepeat="no-repeat";
    document.body.style.backgroundSize="cover";
    setTimeout(aa,4000);
     function aa()
    {
     window.open(""+val[a],"_self");
    }
  }
</script>
<section id="data">

<div class="row">

  <div class="column">
    <div class="card" >
      <p>Earthen Items</p>
      <img src="CST project\Earthen items\download (1).jpg" onclick="bigme(1)">
    </div>
  </div>
 
  <div class="column">
    <div class="card">
      
      <p>Gifted Items</p>
      <img src="CST project\gifted items\download (1).jpg" onclick="bigme(2)">
    </div>
  </div>

  <div class="column">
    <div class="card">
      
      <p>Jute Items</p>
    <img src="CST project\jute items\download (1).jpg" onclick="bigme(3)">
    </div>
  </div>

  <div class="column">
    <div class="card">
      
      <p>Light Items</p>
     <img src="CST project\light items\images (8).jpg"  onclick="bigme(4)">
    </div>
  </div>


</section>
<style type="text/css">
	#b1,#b2,#b3
	{
     font-size: 25px;
	}
	#bb1,#bb2 ,#bb3
	{
		font-size: 45px;
	}
</style>
<section>
<div id="b1" style=" background:url(https://png.pngtree.com/thumb_back/fh260/back_pic/02/68/42/5957908fe6d9740.jpg);">
	<p id="bb1">Indian Handicrafts online on Spectrahut</p>
Online Shopping for Indian Handicraft and Indian Ethic products with Spectrahut.com started the journey back in 2014 with a sole
vision of empowering Indian handicraft artist and small scale women entrepreneurs working in the field of art, craft and fashion.
Indian buyers are getting more attracted towards handmade and unique products that suits their lifestyle, tradition and values.
Spectrahut.com is a small effort to provide a common platform for those who make unique / traditional products as well as for those who
wish to purchase them.<br><br>
</div>
<div  id="b2" style=" background:url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSNshvkAHm1L1dYpx0cZIIIt4DtD6ADvB2muTm1mCpRf_ZnuCLMyQ);">
	<p id="bb2">Buy Indian Handicrafts online in India</p><br>

We celebrate India and endeavour to bring its beautiful art to a global platform. With Spectrahut marketplace
we encourage small entrepreneurs to showcase their creations and to bring them to the global market through internet. If you have a
passion to great cool stuff we have a space to showcase that to the world. Turn your passion into a business with us and watch it grow.<br><br>
</div>
<div  id="b3"  style="background:url(http://gdj.graphicdesignjunction.com/wp-content/uploads/2011/02/background-pattern-design-90.jpg);">
	<p id="bb3">Indian Handicraft store online</p><br>
With Spectrahut we thrive to create a community for creators and entrepreneurs making ethnic products like Indian sarees, Lenghas, Salwar Kameez and kurtis using native Indian crafts. It’s a pleasure to see how these artists create latest fashion trends using our native ethnic craft and make ethnic yet fashionable products like gowns, kurtis, tops, tunics, skirts, jackets, formals, casuals and fusion dresses.<br><br><br>
</div>
</section>








    <div id="foot" class="footer-dark">
        <footer>
            <div class="container">
                <div class="row"> 
                    <div class="col-sm-6 col-md-3 item">
                        <h3>Services</h3>
                        <ul>
                            <li><a href="#">Web design</a></li>
                            <li><a href="#">Development</a></li>
                            <li><a href="#">Designing</a></li>
                            <li style="color:rgb(146,157,164);"></li>
                        </ul>
                    </div>
                    <div class="col-sm-6 col-md-3 item">
                        <h3>About</h3>
                        <ul>
                            <li><a href="#">Company</a></li>
                            <li><a href="#">Team</a></li>
                            <li><a href="#">Careers</a></li>
                        </ul>
                    </div>
                    <div class="col-md-6 item text">
                        <h3 style="font-size:25px;">VIREKHA</h3>
                        <p>Give me a miss call on&nbsp;</p>
                        <p>+896884748<br></p>
                        <p>guptashiv84@gmail.com</p>
                    </div>
                    <div class="col item social "><a href="#"><i class="icon ion-social-facebook "></i></a><a href="#"><i class="icon ion-social-twitter "></i></a><a href="#"><i class="icon ion-social-snapchat "></i></a><a href="#"><i class="icon ion-social-instagram "></i></a></div>
                </div>
                <p class="copyright">Virekha © 2018</p>
            </div>
        </footer>
    </div>

    <script src="assets/js/jquery.min.js"></script>
    <script src="assets/bootstrap/js/bootstrap.min.js"></script>
    <script src="assets/js/-Fixed-Navbar-start-with-transparency-background-BS4-.js"></script>
    <script src="assets/js/Bootstrap-4-Navbar-with-Logo-Image-Brand-Name-and-Responsive-Menu.js"></script>
    <script src="assets/js/Bootstrap-4-Navbar-with-Logo-Image-Brand-Name-and-Responsive-Menu.js"></script>
    <script src="assets/js/Bootstrap-4-Navbar-with-Logo-Image-Brand-Name-and-Responsive-Menu.js"></script>
    <script src="assets/js/MUSA_navbar-metro.js"></script>
    <script src="assets/js/SO-Floating-Nav_v10.js"></script>
    <script type="search.js"></script>
   </body>
   <script>

</script>
</div>
</section>
</body>
</html>
